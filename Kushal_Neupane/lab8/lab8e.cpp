...
int main() {
    std::fstream file("bank.dat", std::ios::binary | std::ios::in | std::ios::out);
    if (!file) {
    ...
    int choice;
    do {
        std::cout << "\n=== TRANSACTION MANAGEMENT SYSTEM ===" << std::endl;
        ...
        if (choice == 1) {
            ...
            if (acc.accountNumber == 0) {
                ...
                Account emptyAcc;
                file.seekp((accNum - 1) * sizeof(Account));
                file.write(reinterpret_cast<char*>(&emptyAcc), sizeof(Account));
